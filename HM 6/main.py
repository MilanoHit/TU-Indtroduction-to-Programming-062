        ...
        while True:
            ...
            try:
                if choice == "1":
                    ...
                    curr = input("Enter how much you want to deposit: ")
                    user.find_by_Iban(Iban).deposit(curr)
                elif choice == "6":
                    user_egn = input("Enter user egn: ")
                    if len(user_egn) != 10 or not user_egn.isdigit():
                        raise InvalidUserData("Invalid EGN number")
                    user = bank.find_user(user_egn)
                    Iban = input("Enter IBAN: ")
                    curr = input("Enter how much you want to withdraw: ")
                    user.find_by_Iban(Iban).withdraw(curr)
                elif choice == "7":
                    print("Goodbye\n")
                    break
                else:
